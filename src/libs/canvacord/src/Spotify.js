...
class Spotify {
  ...
  setImage(source) {
    if (!source)
      throw new Error(
        `Expected image source, received ${typeof title}!`,
      );
    this.image = source;
    return this;
  }
  ...
  setAuthor(name) {
    if (!name || typeof name !== "string")
      throw new Error(
        `Expected artist name, received ${typeof name}!`,
      );
    this.artist = name;
    return this;
  }
  ...
  setAlbum(name) {
    if (!name || typeof name !== "string")
      throw new Error(
        `Expected album name, received ${typeof name}!`,
      );
    this.album = name;
    return this;
  }
  ...
  setStartTimestamp(time) {
    if (!time)
      throw new Error(`Expected timestamp, received ${typeof time}!`);
  ...
  async build() {
    ...
    const progressF = Util.formatTime(
    ...
    await Util.renderEmoji(
      ctx,
      `by ${Util.shorten(this.artist, 40)}`,
      170,
      70,
    );
    ...
    ctx.fillRect(
      170,
      110,
      this.__calculateProgress(progress, total),
      4,
    );
