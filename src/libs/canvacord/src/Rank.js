...
class Rank {
  ...
  setOverlay(color, level = 0.5, display = true) {
    ...
    this.data.overlay.level =
      level && typeof level === "number" ? level : 0.5;
    return this;
  }
  ...
  setRank(data, text = "RANK", display = true) {
    if (typeof data !== "number")
      throw new Error(
        `Level data must be a number, received ${typeof data}!`,
      );
  ...
  async build(ops = { fontX: "Manrope", fontY: "Manrope" }) {
    if (typeof this.data.currentXP.data !== "number")
      throw new Error(
        `Expected currentXP to be a number, received ${typeof this
          .data.currentXP.data}!`,
      );
    if (typeof this.data.requiredXP.data !== "number")
      throw new Error(
        `Expected requiredXP to be a number, received ${typeof this
          .data.requiredXP.data}!`,
      );
    ...
    if (this.data.level.display && !isNaN(this.data.level.data)) {
      ...
      ctx.fillText(
        ...
        800 -
          ctx.measureText(
            Util.toAbbrev(parseInt(this.data.level.data)),
          ).width,
        82,
      );
    ...
    ctx.fillText(
      ...
      670 +
        ctx.measureText(Util.toAbbrev(this.data.currentXP.data))
          .width +
        15,
      164,
    );
