...
class Canvacord {
  ...
  static async burn(image, lvl = 1) {
    ...
    return await Convolute(
      image,
      Canvacord.CONVOLUTION_MATRIX.BURN,
      true,
      lvl,
    );
  }
  ...
  static async beautiful(image) {
    ...
    const base = await Canvas.loadImage(
      Canvacord.assets("IMAGE").BEAUTIFUL,
    );
  ...
  static async facepalm(image) {
    ...
    let layer = await Canvas.loadImage(
      Canvacord.assets("IMAGE").FACEPALM,
    );
  ...
  static async distracted(image1, image2, image3 = null) {
    ...
    const avatar1 = await Canvas.loadImage(
      await Canvacord.circle(image1),
    );
    const avatar2 = await Canvas.loadImage(
      await Canvacord.circle(image2),
    );
  ...
  static async opinion(avatar, msg) {
    ...
    await this.__wait();
    const bg = await Canvas.loadImage(
      Canvacord.assets("IMAGE").OPINION,
    );
  ...
  static async changemymind(text) {
    ...
    await this.__wait();
    const base = await Canvas.loadImage(
      Canvacord.assets("IMAGE").CHANGEMYMIND,
    );
  ...
  static async clyde(message) {
    ...
    let badge = await Canvas.loadImage(
      Canvacord.assets("IMAGE").BOTBADGE,
    );
  ...
  static async quote(options = { image, message, username, color }) {
    await this.__wait();
    if (!options.image)
      options.image = Canvacord.assets("IMAGE").CLYDE;
    ...
    let image = await Canvas.loadImage(
      await Canvacord.circle(options.image),
    );
    ...
    await Util.renderEmoji(
      ctx,
      Util.shorten(options.message, 66),
      230,
      150,
    );
    ...
    ctx.font = "50px Whitney";
    ctx.fillStyle =
      typeof options.color == "string" ? options.color : "#FFFFFF";
    ctx.textAlign = "start";
    ctx.fillText(
      typeof options.username === "string"
        ? Util.shorten(options.username, 17)
        : "Clyde",
      230,
      80,
    );
    ...
    ctx.fillText(
      Util.discordTime(),
      240 +
        ctx.measureText(Util.shorten(options.username, 17)).width +
        110,
      80,
    );
  ...
  static async phub(
    ...
    let baseImage = await Canvas.loadImage(
      Canvacord.assets("IMAGE").PHUB,
    );
    ...
    await Util.renderEmoji(
      ctx,
      Util.shorten(options.message, 50),
      30,
      430,
    );
  ...
  static async wasted(image) {
    ...
    await this.__wait();
    const img = await Canvas.loadImage(
      await Canvacord.greyscale(image),
    );
    const bg = await Canvas.loadImage(
      Canvacord.assets("IMAGE").WASTED,
    );
  ...
  static async youtube(
    ...
    if (!ops.avatar)
      throw new Error("Avatar source may not be empty!");
    ops.dark = !!ops.dark;
    ...
    const avatar = await Canvas.loadImage(
      await Canvacord.circle(ops.avatar),
    );
  ...
  static async reply(
    ...
    ctx.fillText(
      user2,
      165 + usernameWidth + repliedWidth + 20,
      167 - 20,
    );
