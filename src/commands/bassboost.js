...
export default {
  data: {
    name: "bassboost",
    description: "啟動低音加強音效",
    options: [
      {
        type: ApplicationCommandOptionType.String,
        name: "name",
        description:
          "YouTube上的音樂名稱/網址，使用這個值來直接播放加重低音的音樂",
        required: false,
      },
    ],
  },
  run: async function (interaction) {
    if (interaction.client.config.optimizeQuality) {
      const optimizeEmbed = new EmbedBuilder()
        .setTitle("❌ ┃ 為了優化音樂品質，低音加強已停用")
        .setDescription(
          "如果你還是想要啟用低音加強音效，請嘗試[自己建立一個黑貓](https://github.com/blackcatbot/blackcat)",
        )
        .setColor(danger);
      return interaction
        .reply({
    ...
    if (player.songs.length !== 0) player.bassboost(interaction);
    else if (player.songs.length === 0 && url) {
      await interaction.deferReply().catch(() => {});
      player.play(url, interaction, false, false, true);
    } else if (!url && player.songs.length === 0) {
      player.bassboost(interaction);
